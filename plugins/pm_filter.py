    ...
    if 2 < len(message.text) < 100:    
        ...
        if files:
            for file in files:
                ...
                btn.append(
                    [InlineKeyboardButton(text=f"{filename}",callback_data=f"uploader4#{file_id}")]
                    )
        else:
    ...
    if 2 < len(message.text) < 50:    
        ...
        if files:
            for file in files:
                ...
                filename = f"[{get_size(file.file_size)}] {file.file_name}"
                btn.append(
                    [InlineKeyboardButton(text=f"{filename}", url=f"https://telegram.dog/{nyva}?start=uploader4_-_-_-_{file_id}")]
                )
        else:
            return
        if not btn:
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if (clicked == typed):
        ...
        if query.data.startswith("next"):
        ...
        elif query.data.startswith("uploader4"):
            ...
            if AUTH_CHANNEL and not await is_subscribed(client, query):
                await query.answer("fucker",show_alert=True)
                return
            ident, file_id = query.data.split("#")
            filedetails = await get_file_details(file_id)
            for files in filedetails:
